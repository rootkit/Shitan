...
- (void)layoutSubviews
{
    ...
    self.imageView.layer.cornerRadius = 6;//设置那个圆角的有多圆
...
- (void)layoutSubviews
{
    ...
    self.titleLabel.frame = (CGRect){CGPointZero, CGSizeMake(self.bounds.size.width, 5)};
    ...
    [self.cancelButton setBackgroundImage:@"btn36_white.png" setSelectedBackgroundImage:@"btn29_grey.png"];
    
    self.cancelButton.frame = CGRectMake(self.bounds.size.width*0.05, self.titleLabel.bounds.size.height + self.contentScrollView.bounds.size.height, self.bounds.size.width*0.9, 36);
    
    self.bounds = (CGRect){CGPointZero, CGSizeMake(self.bounds.size.width, self.titleLabel.bounds.size.height + self.contentScrollView.bounds.size.height + 50)};
}

- (void)layoutContentScrollView
{
    UIEdgeInsets margin = UIEdgeInsetsMake(0, 10, 15, 10);
    CGSize itemSize = CGSizeMake((self.bounds.size.width - margin.left - margin.right) / 4, 90);
