...
#import "RECustomPickerCell.h"
#import "RETableViewManager.h"
#import "NSString+RETableViewManagerAdditions.h"

@interface RECustomPickerCell ()
...
@implementation RECustomPickerCell

@synthesize item = _item;

+ (BOOL)canFocusWithItem:(RECustomPickerItem *)item
{
...
- (void)loadPlistData
{
    ...
    self.district = [self.areas objectAtIndex:0];
}
...
#endif
        self.item.inlinePickerItem = [REInlineCustomPickerItem itemWithPickerItem:self.item];
...
{
    self.item.value = self.valueArray;
    self.valueLabel.text = self.item.value ? [self.item.value componentsJoinedByString:@", "] : @"";
    self.placeholderLabel.hidden = self.valueLabel.text.length > 0;
}
...
- (void)setItem:(RECustomPickerItem *)item
{
    if (_item != nil) {
        [_item removeObserver:self forKeyPath:@"enabled"];
    }
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
    ...
    switch (component) {
        case 0:
        {
            ...
            self.state = [[self.provinces objectAtIndex:row] objectForKey:@"state"];
            self.city = [[self.cities objectAtIndex:0] objectForKey:@"city"];
            self.district = [self.areas objectAtIndex:0];
        }
            break;
        ...
        case 2:
            self.district = [self.areas objectAtIndex:row];
            break;
    ...
    [self.valueArray addObject:self.state];
    [self.valueArray addObject:self.city];
    [self.valueArray addObject:self.district];
