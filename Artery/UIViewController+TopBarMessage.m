...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        self.label = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 50, 35)];
...
- (void)resetViews
{
    if (!__defaultTopMessageConfig) {
        __defaultTopMessageConfig = [@{kDXTopBarBackgroundColor:[UIColor colorWithRed:224.0/255.0 green:57.0/255.0 blue:65.0/255.0 alpha:0.70], kDXTopBarTextColor : [UIColor whiteColor], kDXTopBarTextFont : [UIFont fontWithName:@"HelveticaNeue-Medium" size:14.0]} mutableCopy];
    }
...
- (void)layoutSubviews
{
    ...
    self.label.frame = CGRectMake((CGRectGetWidth(self.bounds) - textSize.width) * 0.5, (CGRectGetHeight(self.bounds) - labelDefaultHeight) * 0.5, textSize.width, labelDefaultHeight);
    
    
    //图片
    CGFloat iconHight  = 14.0f;
    CGFloat iconWidth = 8.0f;
    
    self.iconIgv.frame = CGRectMake(CGRectGetWidth(self.bounds) - iconWidth - 14, (CGRectGetHeight(self.bounds) - iconHight) * 0.5, iconWidth, iconHight);
