...
#define CANCEL_BUTTON_COLOR                     [UIColor grayColor]
...
#define ACTIONSHEET_BACKGROUNDCOLOR             [UIColor colorWithRed:0.967 green:1.000 blue:0.974 alpha:0.85]
...
#define BUTTON_WIDTH                            MAINSCREEN.size.width - 30*2

#define BUTTONTITLE_FONT                        [UIFont systemFontOfSize:18.0]
...
#define TITLE_WIDTH                             MAINSCREEN.size.width - 30*2
...
@property (nonatomic, unsafe_unretained) id<LXActionSheetDelegate>delegate;
...
- (UIButton *)creatOtherButtonWith:(NSString *)otherButtonTitle withPostion:(NSInteger )postionIndex
{
    ...
    UIImage *image1 =[UIImage imageNamed:@"btn36_white.png"];
    image1 = [image1 stretchableImageWithLeftCapWidth:4 topCapHeight:4];
    [otherButton setBackgroundImage:image1 forState:UIControlStateNormal];
    
    UIImage *image2 =[UIImage imageNamed:@"btn38_grey_p.png"];
    image2 = [image2 stretchableImageWithLeftCapWidth:4 topCapHeight:4];
    [otherButton setBackgroundImage:image2 forState:UIControlStateSelected];
    [otherButton setBackgroundImage:image2 forState:UIControlStateHighlighted];
...
- (UIButton *)creatCancelButtonWith:(NSString *)cancelButtonTitle
{
    ...
    UIImage *image1 =[UIImage imageNamed:@"btn38_grey.png"];
    image1 = [image1 stretchableImageWithLeftCapWidth:4 topCapHeight:4];
    [cancelButton setBackgroundImage:image1 forState:UIControlStateNormal];
    
    UIImage *image2 =[UIImage imageNamed:@"btn38_grey_p.png"];
    image2 = [image2 stretchableImageWithLeftCapWidth:4 topCapHeight:4];
    [cancelButton setBackgroundImage:image2 forState:UIControlStateSelected];
    [cancelButton setBackgroundImage:image2 forState:UIControlStateHighlighted];
...
- (void)clickOnButtonWith:(UIButton *)button
{
    ...
    if (self.delegate) {
        if ([self.delegate respondsToSelector:@selector(actionSheet:didClickOnButtonIndex:)] == YES) {
            [self.delegate actionSheet:self didClickOnButtonIndex:(int)button.tag];
        }
    }
